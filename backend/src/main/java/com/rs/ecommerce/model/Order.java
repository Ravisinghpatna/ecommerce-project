...
/**
 * Order entity — jab user "checkout" karta hai to cart ke items se ek Order +
 * uske OrderItems ban jaate hain, aur cart khaali ho jaata hai. Ye e-commerce
 * ka standard flow hai: Cart (temporary) -> Order (permanent record).
 */
@Entity
@Table(name = "orders")
public class Order {
	...
	// Simple order status tracking (real app me ye enum ho sakta hai: PLACED,
	// SHIPPED, DELIVERED...)
	@Column(nullable = false)
	...
	// Kaunsi courier company se bheja gaya (BLUE_DART, DELHIVERY, SPEED_POST, DTDC)
...
//Courier ke detailed tracking messages (jaise real tracking page pe dikhte hain)
	@OneToMany(mappedBy = "order", cascade = CascadeType.ALL, orphanRemoval = true)
	@OrderBy("timestamp ASC")
	...
	// mappedBy = "order" batata hai ki relationship OrderItem class ke `order`
	// field se control hoti hai
	...
	// Helper: tracking update add karo, dono taraf se relationship set karke
	public void addTrackingUpdate(String message, LocalDateTime when) {
	    trackingUpdates.add(new TrackingUpdate(this, message, when));
	}
